import { Link } from "react-router-dom";
import heroImg from "../../../assets/images/hero.jpg";

const Hero = () => {
  return (
    <section className="xl:min-h-[calc(100vh-100px)] flex justify-center items-center bg-base-100">
      <div className="my-8 md:my-10 rounded-xl px-6 md:px-8 py-10 md:py-20 xl:pt-28 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        {/* Left content */}
        <div>
          <h1 className="text-4xl md:text-6xl xl:text-7xl font-bold text-gray-900 leading-tight">
            Donate <span className="text-red-600">Blood</span>, Save Lives in Chittagong
          </h1>
          <p className="mt-7 xl:mt-10 text-gray-600 font-medium max-w-md">
            CTG Blood Connect helps people in Chittagong find blood donors quickly
            and easily. Join as a donor and make a real impact in your community.
          </p>

          <div className="mt-10 xl:mt-12 flex flex-wrap gap-4">
            <Link to={'/be-a-donor'} className="cursor-pointer px-6 font-medium py-2.5 bg-red-400 text-gray-100 rounded-lg">
              Become a Donor
            </Link>
            <Link to={'/donors'} className="transition-all duration-300 cursor-pointer px-6 font-medium py-2.5 bg-white border-2 border-red-400 text-black hover:bg-red-400 hover:text-gray-100 rounded-lg">
              Find a Donor
            </Link>
          </div>
        </div>
        {/* Right image */}
        <div className="flex justify-center lg:justify-end">
          <img
            src={heroImg}
            alt="Blood Donation"
            className="max-w-md lg:max-w-xl 2xl:max-w-2xl w-full rounded-l-lg"
          />
        </div>
      </div>
    </section>
  );
};

export default Hero;
